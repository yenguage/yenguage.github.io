...
구체적인 방법은 다음과 같다. 기본적으로 지금 우리가 관심이 있는 target user와 item은 각각 0과 1을 부여한다. 그리고 subgraph 내의 나머지 노드들에 대해서 user이면 짝수 $2i$ 로, item이면 홀수 $2i+1$ 로 번호를 부여한다. 이때 $i$ 는 hop이다. 즉, target 노드로부터 얼마나 멀리 떨어져 있는지를 고려한다.
